...
const DecentralizedBank =
  artifacts.require('./dBank')
...
contract('dBank', ([deployer, user]) => {
  ...
  beforeEach(async () => {
    ...
    dbank = await DecentralizedBank.new(
      token.address
    )
  ...
  describe('testing token contract...', () => {
    describe('success', () => {
      ...
      it('checking token symbol', async () => {
        expect(await token.symbol()).to.be.eq(
          'DBC'
        )
      })
      ...
      it('dBank should have Token minter role', async () => {
        expect(await token.minter()).to.eq(
          dbank.address
        )
      })
    })
  ...
  describe('testing deposit...', () => {
    ...
    describe('success', () => {
      ...
      it('balance should increase', async () => {
        expect(
          Number(await dbank.etherBalanceOf(user))
        ).to.eq(10 ** 16)
      })
  ...
  describe('testing withdraw...', () => {
    ...
    describe('success', () => {
      ...
      it('user should receive proper amount of interest', async () => {
        ...
        expect(balance % interestPerSecond).to.eq(
          0
        )
        expect(balance).to.be.below(
          interestPerSecond * 4
        )
      })
  ...
  describe('testing borrow...', () => {
    describe('success', () => {
      ...
      it('token total supply should increase', async () => {
        expect(
          Number(await token.totalSupply())
        ).to.eq(5 * 10 ** 15) //10**16/2
      })

      it('balance of user should increase', async () => {
        expect(
          Number(await token.balanceOf(user))
        ).to.eq(5 * 10 ** 15) //10**16/2
      })
      ...
      it('collateralEther should increase', async () => {
        expect(
          Number(
            await dbank.collateralEther(user)
          )
        ).to.eq(10 ** 16) //0.01 ETH
      })

      it('user isBorrowed status should eq true', async () => {
        expect(
          await dbank.isBorrowed(user)
        ).to.eq(true)
      })
    })
